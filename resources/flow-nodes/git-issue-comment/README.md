# @particle-academy/git_issue_comment
...
```bash
npx fancy-cli@latest add node @particle-academy/git_issue_comment
```
...
```ts
import { registerGitHost } from "@/components/fancy/flow-nodes/git-issue-comment/js/provider";
...
```ts
import { gitIssueCommentRunnableKind } from "@/components/fancy/flow-nodes/git-issue-comment/js/kind";

registerNodeKind(gitIssueCommentRunnableKind);
```
