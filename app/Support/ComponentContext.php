...
class ComponentContext
{
    ...
    private const ENTRIES = [
        ...
        'fancy-map/map' => [
            'why' => 'Maps are where "put a pin on it" meets a pile of engine-specific ceremony: Leaflet and Google have different APIs, different coordinate orders, different marker plumbing, and both fight React&apos;s render cycle — so most apps hard-wire one engine, hand-roll a controlled wrapper, and then have no clean way to move markers live or to let an <em>agent</em> touch the map without scraping the DOM. <code>Map</code> collapses all of that into one controlled surface: swap OpenStreetMap for Google with a one-line <code>provider</code> change, drive live tracking off plain state, and cohabit the same map with an agent over an MCP bridge.',
            'what' => 'An engine-agnostic <code>&lt;Map&gt;</code> whose <code>view</code> + <code>markers</code> + <code>selectedId</code> are controlled, JSON-friendly props (an agent can emit a marker array directly). Providers are swappable — <code>leafletProvider()</code> (OpenStreetMap or any XYZ raster source, no key) and <code>googleProvider({ apiKey })</code> — with no map-engine dependency in the core. <code>follow</code> keeps the camera on a moving marker; <code>useGeolocationTrack()</code> wires the browser&apos;s live position; every pin carries a stable <code>data-map-marker-id</code> handle. SSR-safe — a sized placeholder renders on the server and the engine mounts in an effect. <code>registerMapBridge</code> makes it inhabitable: an agent pans, drops/updates/removes markers, selects, fits bounds, and follows a track, with every mutation broadcasting <code>AgentActivity</code> and pushing an undo entry.',
            'how' => 'Install <code>@particle-academy/fancy-map</code> plus the engine you want (<code>leaflet</code> or <code>@googlemaps/js-api-loader</code>). Give the map a container with a real height and, for Leaflet, import its stylesheet once: <code>import "leaflet/dist/leaflet.css"</code>. Then <code>&lt;Map provider={leafletProvider()} view={view} onViewChange={setView} markers={markers} selectedId={id} onSelect={setId} /&gt;</code>. Move to Google by swapping only the <code>provider</code>. For live tracking, feed <code>markers</code> new positions (a websocket, an Echo channel, or <code>useGeolocationTrack</code>) and set <code>follow</code> to the moving marker&apos;s id. For a cohabited human/agent map, wire <code>registerMapBridge(host, { adapter: { getView, setView, getMarkers, setMarkers, getSelected, setSelected, fitBounds, setFollow } })</code> from <code>@particle-academy/agent-integrations/bridges/map</code> against the same state — see the demo at <code>/react-demos/map-agent</code>.',
        ],
