# @particle-academy/git_issue_open
...
```bash
npx fancy-cli@latest add node @particle-academy/git_issue_open
```
...
```ts
import { registerGitHost } from "@/components/fancy/flow-nodes/git-issue-open/js/provider";
...
```ts
import { gitIssueOpenRunnableKind } from "@/components/fancy/flow-nodes/git-issue-open/js/kind";

registerNodeKind(gitIssueOpenRunnableKind);
```
