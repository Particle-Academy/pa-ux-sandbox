...
2. **`npx @particle-academy/fancy-ui add`.** The shadcn-style vendoring path. The CLI fetches the component source from our [hosted registry](/docs/registry) and writes the files into your codebase under `src/components/fancy/{slug}/`. You own the source. You can fork freely. Updates are explicit, never automatic.
