# @particle-academy/git_issue_update
...
```bash
npx fancy-cli@latest add node @particle-academy/git_issue_update
```
...
```ts
import { registerGitHost } from "@/components/fancy/flow-nodes/git-issue-update/js/provider";
...
```ts
import { gitIssueUpdateRunnableKind } from "@/components/fancy/flow-nodes/git-issue-update/js/kind";

registerNodeKind(gitIssueUpdateRunnableKind);
```
