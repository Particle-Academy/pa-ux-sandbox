...
namespace FancyFlow\Nodes\GitIssueComment;
...
#[FlowNode(
    name: '@particle-academy/git_issue_comment',
    category: 'io',
...
final class GitIssueCommentExecutor implements NodeExecutor
{
    ...
    public function execute(ExecutionContext $ctx): mixed
    {
        ...
        if ($number === 0) {
            throw new \RuntimeException('git_issue_comment: needs an issue "number" — on the node, or from the input.');
        }

        $body = trim((string) ($config['body'] ?? ''));
        if ($body === '') {
            throw new \RuntimeException('git_issue_comment: needs a "body". Refusing to post an empty comment.');
        }

        $comment = $provider->commentOnIssue($ref, $number, $body);

        $ctx->emit(RunEvent::log('info', 'commented on #'.$number, $ctx->node->id));

        return Port::only('out', ['number' => $number] + $comment);
    }
