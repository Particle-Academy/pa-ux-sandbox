# @particle-academy/git_issue_list
...
```bash
npx fancy-cli@latest add node @particle-academy/git_issue_list
```
...
```ts
import { registerGitHost } from "@/components/fancy/flow-nodes/git-issue-list/js/provider";
...
```ts
import { gitIssueListRunnableKind } from "@/components/fancy/flow-nodes/git-issue-list/js/kind";

registerNodeKind(gitIssueListRunnableKind);
```
